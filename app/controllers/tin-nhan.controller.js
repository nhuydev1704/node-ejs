...
class TinNhanCtrl {
  index(req, res) {
    ...
    QLTN.getAll(content, (err, data) => {
      console.log(
        "🚀 ~ file: tin-nhan.controller.js:13 ~ TinNhanCtrl ~ QLTN.getAll ~ data:",
        data
      );
      if (err) res.redirect("/500");
      else res.render("tin-nhan/index", { data: data });
    });
  }
  ...
  taoTin(req, res) {
    QLUser.getAll(req, (err, data) => {
      if (err) res.redirect("/500");
      else res.render("tin-nhan/create", { data: data });
    });
  }
  ...
  create(req, res) {
    console.log(
      "🚀 ~ file: tin-nhan.controller.js:34 ~ TinNhanCtrl ~ create ~ req.body:",
      req.body
    );
