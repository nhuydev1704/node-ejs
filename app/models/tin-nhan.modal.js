...
class QLTN {
    static insert(tinNhan, callback) {
        console.log('🚀 ~ file: tin-nhan.modal.js:16 ~ QLTN ~ insert ~ tinNhan:', tinNhan);
        sql.query(
  ...
  WHERE 
    ...
    static update(id, tinNhan, callback) {
        sql.query('UPDATE message SET ? WHERE id = ?', [tinNhan, id], (err, res) => {
            if (err) {
                console.log('error: ', err);
                callback(null, err);
                return;
    ...
    static delete(ids, req, callback) {
        ...
        const query = `UPDATE message SET deleted_at = ${JSON.parse(req.cookies.user).id} WHERE id IN (${idsString})`;
